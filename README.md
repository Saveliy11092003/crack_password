...
![image](https://github.com/user-attachments/assets/da086b50-0885-42d1-800d-9e33c1c4c92c)
...
2. **Менеджер** обрабатывает запрос и генерирует задачи в соответствии с заданным числом воркеров на поиск возможного пароля. После чего отправляет их на исполнение **воркерами** через первый RabbitMQ. 
3. Каждый **воркер** принимает задачу, перебирает слова в заданном диапазоне и вычисляет их хэш. Находит слова у которых хеш совпадает с заданным, и результат работы возвращает **менеджеру** уже через второй RabbitMQ.
...
MongoDB используется для CRUD операций с задачами, позволяет не терять информацию об задачах.
...
Для обработки ответов от **воркера** в **менеджере** реализован слушатель очереди RabbitMQ.  
Сообщение формируется на основе [xsd-схемы](https://drive.google.com/file/d/14tliqQQCTxg0HwrdPyxVQjVQYMC0o8zl/view).  

## ⚡ API воркера  
**Воркеры** слушают RabbitMQ и принимают свои задачи от **менеджера**, который отсылает подзадачи в RabbitMQ.
